...
summary: executable art-direction skill for frontend agents building distinctive brand sites, product interfaces, tool UIs, and editorial experiences with strong visual identity, anti-template safeguards, and consistency controls.
when_to_use:
  - when the brief calls for a point of view instead of generic SaaS patterns
  - when composition, hierarchy, typography, motion, imagery, and brand specificity matter
  - when the interface should feel original without sacrificing usability
  - when the agent must generate distinct visual directions before building
  - when the risk of derivative, framework-default, or template-like output is high
when_not_to_use:
  - tiny utility fixes that do not change structure or visual language
  - routine CRUD/admin pages where speed and clarity matter more than art direction
  - highly regulated surfaces where experimentation would harm trust or compliance
version: 3.0
---
